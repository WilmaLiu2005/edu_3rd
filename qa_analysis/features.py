...
from .feature_utils import replace_inf_with_reasonable_value, contains_copy_keywords, count_copy_keywords, COPY_KEYWORDS
from .config import ANCHOR_DATE

WHY_KEYWORDS = [
    "为什么", "为啥", "为何", "因为什么", "怎么会", "怎么还", 
    "是因为", "原因是什么", "原因是啥", "导致了什么", "造成了什么", "怎么造成",
    "怎么出现", "为什么会", "怎么变成", "为什么这样"
]
...
def extract_features_from_dialog(file_path, df_class, df_homework, df_schedule, df_school=None, 
    ...
    try:
        ...
        if '提问入口' in df.columns:
            ...
            is_video_unit = int((df["提问入口"] == "视频单元").any()) if qa_turns > 0 else 0
            is_discussion_unit = int((df["提问入口"] == "讨论单元").any()) if qa_turns > 0 else 0
            is_graphic_unit = int((df["提问入口"] == "图文单元").any()) if qa_turns > 0 else 0
            is_ai_task = int((df["提问入口"] == "课堂AI任务").any()) if qa_turns > 0 else 0
            is_confusion_entry = int(df["提问入口"].isin(["课堂不懂", "课件不懂", "习题不懂"])) if qa_turns > 0 else 0
        else:
            print(f"Warning: File missing entry point column: {file_name}")
            if_non_class = 0
            is_video_unit = 0
            is_discussion_unit = 0
            is_graphic_unit = 0
            is_ai_task = 0
        ...
        for q in df["提问内容"]:
            if not isinstance(q, str):
                continue
            # 检测图片上传模式
            if "![用户上传图片](" in q and not is_confusion_entry:
                is_copy_paste = 1
                copy_keywords_count += 1
            # 统计关键词数量（包括普通关键词和正则模式）
            copy_keywords_count += count_copy_keywords(q)
            if not is_copy_paste and contains_copy_keywords(q):
                is_copy_paste = 1  # 一旦发现复制迹象即可标记
        ...
        features = {
            ...
            "is_copy_paste": int(is_copy_paste),
            "copy_keywords_count": int(copy_keywords_count),
            "is_video_unit": int(is_video_unit),
            "is_discussion_unit": int(is_discussion_unit),
            "is_graphic_unit": int(is_graphic_unit),
            "is_ai_task": int(is_ai_task),
            "is_confusion_entry": int(is_confusion_entry),
        }
        ...
        question_type_why = int(any(kw in question_texts for kw in WHY_KEYWORDS))
        ...
        features.update({
            ...
            "question_type_why": int(question_type_why)
        })
